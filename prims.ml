  ...
  let make_routine label body =
    label ^ ":
       ...
       " ^ body ^ "
         pop rbp
         ret";;
  (*This identity function is designed for apply asm code we implemented down bellow*)
  let make_identity label body = 
    label ^ ":
    " ^ body;;
  ...
  let misc_ops =
    let misc_parts = [
	 ...
	 mov rdx, rax
       ...
       (* Our implementations of apply (variadic), car, cdr, cons, set_car!, set_cdr! *)
        " 
        CAR rax, rsi
        ", make_unary, "car";
        ...
        "
        lea rax, [rsi + TYPE_SIZE]
        mov [rax], rdi
        mov rax, SOB_VOID_ADDRESS
        ", make_binary, "set_car";

        "
        lea rax, [rsi + TYPE_SIZE + WORD_SIZE]
        mov [rax], rdi
        mov rax, SOB_VOID_ADDRESS
        ", make_binary, "set_cdr";
        ...
        "push rbp
        mov rbp, rsp 
        mov rsi, qword [rbp + WORD_SIZE]	    	; last return address
        
        mov r8, qword [rbp + WORD_SIZE*3]       ; r8 <- n
        mov rax, qword [rbp + WORD_SIZE*4]      ; rax <- proc
        mov r14, qword [rbp + (r8 + 3)*WORD_SIZE]     ; r14 <- pointer to list (pointer to a SOB_PAIR (which is a pointer!))
        
        ; %1 is the list we want to count
        ; %2 is a register which will hold the size of the list.
        ; rcx <- |list|
        LIST_LENGTH r14, rcx

        ; %1 is the list we want to count (don't we want to push it?).
        ; %2 list size.
        ; push list in r14 to stack in reverse order		
        PUSH_LIST_TO_STACK r14, rcx

        ; push n - 2 arguments to the stack, if they exist (optional). 
        ; from arg_1 to arg_n-2 (without arg_0 and arg_n-1 which they are proc and list)
        PUSH_ARGS_TO_STACK
        
        sub rcx, 2
        add rcx, r8				      ; number of total arguments in rcx (rcx <- n + |list| - 2)
        push rcx				        ; push number of new arguments to stack
        
        CLOSURE_ENV r11, rax                    ; r11 <- proc ENV
        CLOSURE_CODE r12, rax                   ; r12 <- proc code  

        CHECK_IF_CLOSURE rax         ; Divide by 0 if rax is not a closure.
        
        push r11			             ; push new ENV to stack
        push rsi				          ; push return adress of the previews frame
        lea r8, [r8 + 4]
        SHIFT_UP_TP r8
        jmp r12" , make_identity, "apply";
      ] in
