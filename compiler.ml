...
main:
    ...
    ;; Since the primitive procedures are defined in assembly,
...
try
  ...
  (* Add the next row instead of the one after it!!! *)
  ...
  let generate = Code_Gen.generate consts_tbl fvars_tbl (-1) in 
...
with Invalid_argument(x) -> (print_endline x; raise X_missing_input_file);;
