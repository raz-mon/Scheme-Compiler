...
(define fold-left 
  (lambda (f init li)
	(if (null? li)
		init
		(fold-left f (f init (car li) ) (cdr li) )
	)
  )
)
...
(define all_but_last
	(lambda (li)
		(cond
			((null? li) li)
			((equal? '() (cdr li)) '())
			(else (cons (car li) (all_but_last (cdr li) ) ) )
		)
	)
)
...
(define reverse
	(lambda (li)
		(if (null? li)
			li
			(cons (last_in_list li) (reverse (all_but_last li) ) )
		)
	)
)
...
(define fold-right
	(lambda (f init li)
		(if (null? li)
			init
			(let ((rev (reverse li) ))
 				(fold-right f (f (car rev) init) (reverse (cdr rev) ) )
	 		)
		)
	)
)

(define prop_to_improp
	(lambda (li)
		(if (null? li)
			li
			(if (equal? (length li) 1)
				(car li)
				(cons (car li) (prop_to_improp (cdr li) ) )
			)
		)
	)
)
